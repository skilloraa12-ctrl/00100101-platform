...
const LIBRARY_TERMS = [
  ...
  { name: "біт (bit)", desc: "Найменша одиниця інформації в комп'ютері — один розряд, що може мати лише два значення: 0 або 1.", syntax: "01001000 01101001 → \"Hi\" (8 біт на символ)", attrs: "від binary digit — «двійкова цифра»", pitfalls: "Не плутай з байтом: 1 байт = 8 біт, різниця в 8 разів важлива при розрахунку швидкості інтернету (Мбіт/с vs МБ/с)." },
  { name: "байт (byte)", desc: "Група з 8 біт — базова одиниця виміру обсягу даних, зазвичай представляє один символ тексту.", syntax: "1 байт = 8 біт = число від 0 до 255", attrs: "скорочення: B (байт) vs b (біт)", pitfalls: "Провайдери інтернету рекламують швидкість у мегабітах (Мбіт/с), а розмір файлів показують у мегабайтах (МБ) — це джерело плутанини «чому швидкість менша, ніж очікував»." },
  { name: "кілобайт, мегабайт, гігабайт", desc: "Одиниці виміру обсягу даних більшого масштабу: 1 КБ ≈ 1024 байти, 1 МБ ≈ 1024 КБ, 1 ГБ ≈ 1024 МБ.", syntax: "1 KB = 1024 B\n1 MB = 1024 KB\n1 GB = 1024 MB", attrs: "KB, MB, GB, TB — кіло/мега/гіга/тера", pitfalls: "Виробники накопичувачів рахують 1 ГБ = 1000 МБ (десяткова система), а ОС показує реальний об'єм у 1024-based числах — звідси «зникнення» частини заявленого обсягу диска." },
  { name: "двійковий код (binary)", desc: "Система числення лише з двох цифр (0 і 1), в якій комп'ютер представляє буквально всі дані — числа, текст, зображення, звук.", syntax: "5 (десяткове) = 101 (двійкове)\n2⁰+0+2² = 1+0+4 = 5", attrs: "binary — «двійковий»; основа систем числення = 2", pitfalls: "Комп'ютер оперує лише двійковим кодом на найнижчому рівні — усе інше (текст, кольори, звук) це лише різні способи ІНТЕРПРЕТАЦІЇ тих самих 0 і 1." },
  { name: "шістнадцяткова система (hex)", desc: "Система числення з 16 цифрами (0-9, потім A-F), зручна для компактного запису двійкових даних — кожна hex-цифра відповідає рівно 4 бітам.", syntax: "#FF5733 — колір у CSS\n255 (десяткове) = FF (hex)", attrs: "hex, hexadecimal; префікс # у CSS чи 0x у коді", pitfalls: "Часта пастка новачків — переплутати hex-код кольору з десятковим числом; FF в hex — це 255, а не 15 чи 255000." },
  { name: "ASCII / Unicode", desc: "Стандарти, що зіставляють числа з символами тексту. ASCII підтримує лише 128 базових латинських символів, Unicode — практично всі символи всіх мов світу, включно з емодзі.", syntax: "'A' → 65 (ASCII)\n'😀' → U+1F600 (Unicode)", attrs: "UTF-8 — найпоширеніше кодування Unicode в вебі", pitfalls: "Файл, збережений в одному кодуванні (напр. Windows-1251) і відкритий в іншому (UTF-8) — показує «кракозябри» замість тексту." },
  { name: "пропускна здатність (bandwidth)", desc: "Максимальний обсяг даних, який можна передати мережею за одиницю часу — «ширина труби» для даних, вимірюється в біт/с (Мбіт/с, Гбіт/с).", syntax: "100 Мбіт/с = 100 000 000 біт/с ≈ 12.5 МБ/с", attrs: "throughput — фактична (не максимальна) пропускна здатність", pitfalls: "Висока пропускна здатність не означає низьку затримку (latency) — можна мати «широкий, але повільний» канал зв'язку." },
  { name: "затримка (latency)", desc: "Час, який потрібен одному пакету даних, щоб дійти від відправника до отримувача — «як довго чекати відповіді», на відміну від bandwidth («скільки можна передати одразу»).", syntax: "ping google.com → time=15ms (затримка 15 мілісекунд)", attrs: "ping — утиліта для вимірювання затримки", pitfalls: "Низька затримка критична для ігор і відеодзвінків навіть при високій пропускній здатності — «швидкий, але з паузами» канал відчувається гірше, ніж стабільно повільний." },
  { name: "процесор (CPU)", desc: "«Мозок» комп'ютера — виконує інструкції програм, робить обчислення. Швидкість вимірюється в гігагерцах (GHz), сучасні CPU мають кілька ядер (cores) для паралельної роботи.", syntax: "Intel Core i7, 8 ядер, 3.6 GHz", attrs: "CPU — Central Processing Unit; ядро (core), потік (thread)", pitfalls: "Більше ядер не завжди означає швидше — не кожна програма вміє ефективно розподіляти роботу між кількома ядрами одночасно." },
  { name: "оперативна пам'ять (RAM)", desc: "Швидка тимчасова пам'ять, де зберігаються дані активних програм прямо зараз. Вимикаєш комп'ютер — усе, що було в RAM, зникає (на відміну від диска).", syntax: "16 GB RAM — типовий обсяг для розробки у 2025", attrs: "RAM — Random Access Memory; volatile memory — «летюча» пам'ять", pitfalls: "Замало RAM — комп'ютер починає «підкачувати» дані з повільного диска (swap), і все різко гальмує." },
  { name: "кеш (cache)", desc: "Маленька, але дуже швидка пам'ять, що зберігає копію нещодавно використаних даних, щоб не діставати їх щоразу з повільнішого джерела.", syntax: "CPU cache: L1 → L2 → L3 → RAM (кожен рівень повільніший, але більший)", attrs: "cache hit — дані знайдено в кеші; cache miss — не знайдено", pitfalls: "Застарілі (stale) дані в кеші — класичне джерело багів «я змінив файл, а бачу стару версію»; вирішується інвалідацією кешу." },
  { name: "відеокарта (GPU)", desc: "Процесор, спеціалізований на паралельних обчисленнях — спочатку для графіки й ігор, сьогодні також для навчання ШІ-моделей, де потрібно робити мільйони однакових обчислень одночасно.", syntax: "NVIDIA RTX 4090 — популярна GPU для ігор і ML", attrs: "GPU — Graphics Processing Unit; CUDA — платформа NVIDIA для обчислень на GPU", pitfalls: "GPU швидша за CPU лише для задач, що добре розпаралелюються (графіка, ML) — для звичайного послідовного коду CPU часто ефективніший." },
  { name: "SSD / HDD", desc: "Два типи накопичувачів для постійного зберігання даних. SSD (без рухомих частин) значно швидший і надійніший за старіший HDD (з магнітними дисками, що обертаються).", syntax: "SSD: ~500 MB/с+ · HDD: ~100-150 MB/с", attrs: "SSD — Solid State Drive; HDD — Hard Disk Drive", pitfalls: "SSD дорожчий за гігабайт, ніж HDD — для великих архівів холодних даних HDD усе ще економічно доцільніший." },
  { name: "компілятор vs інтерпретатор", desc: "Компілятор перекладає весь код у машинний код заздалегідь, ДО запуску (C++, Go). Інтерпретатор виконує код рядок за рядком під час запуску, без окремого етапу компіляції (Python, стандартний JS).", syntax: "Компіляція: код → .exe → запуск\nІнтерпретація: код → виконання одразу", attrs: "JIT (just-in-time) — гібрид: компіляція «на льоту» під час виконання", pitfalls: "Скомпільовані мови зазвичай швидші у виконанні, але повільніші в циклі розробки (треба перекомпільовувати після кожної зміни)." },
  { name: "середовище виконання (runtime)", desc: "Програмне середовище, необхідне для виконання коду певної мови — надає базові бібліотеки, керування пам'яттю, зв'язок з операційною системою.", syntax: "Node.js — runtime для виконання JavaScript поза браузером", attrs: "runtime error — помилка, що трапляється САМЕ під час виконання, а не в коді заздалегідь", pitfalls: "Код може бути синтаксично правильним, але падати в runtime (напр. звернення до неіснуючої властивості) — це не те саме, що помилка компіляції/syntax error." },
  { name: "IDE (середовище розробки)", desc: "Програма для написання коду з вбудованими інструментами: підсвічування синтаксису, автодоповнення, дебагер, інтеграція з git — усе в одному вікні, на відміну від простого текстового редактора.", syntax: "VS Code, WebStorm, PyCharm, Xcode", attrs: "IDE — Integrated Development Environment", pitfalls: "IDE — це не те саме, що мова програмування; можна писати JavaScript у будь-якому IDE, а не лише в «спеціальному для JS»." },
  { name: "IP-адреса", desc: "Унікальна числова адреса пристрою в мережі — як домашня адреса, за якою до тебе можуть «доставити» дані. IPv4 (напр. 192.168.1.1) поступово витісняється довшим IPv6.", syntax: "192.168.1.1 (IPv4)\n2001:0db8::1 (IPv6)", attrs: "публічна IP (видно всьому інтернету) vs приватна (лише в локальній мережі)", pitfalls: "Приватні IP (192.168.x.x, 10.x.x.x) не унікальні в усьому світі — вони можуть повторюватись у різних домашніх мережах одночасно." },
  { name: "DNS", desc: "Система, що перетворює зручні для людини доменні імена (google.com) на числові IP-адреси, які реально використовує мережа для маршрутизації.", syntax: "google.com → DNS → 142.250.186.78", attrs: "DNS — Domain Name System; «телефонна книга інтернету»", pitfalls: "Зміни DNS-записів поширюються по всьому світу не миттєво (пропагація може зайняти від хвилин до 48 годин) — сайт може бути доступний одним і недоступний іншим одразу після зміни." },
  { name: "HTTP / HTTPS", desc: "Протокол передачі веб-сторінок між браузером і сервером. HTTPS — той самий протокол, але з шифруванням (S = Secure), що захищає дані від перехоплення по дорозі.", syntax: "http://example.com — незашифровано\nhttps://example.com — зашифровано (TLS/SSL)", attrs: "замок 🔒 у браузері підтверджує HTTPS-з'єднання", pitfalls: "Сайт без HTTPS сучасні браузери позначають як «Небезпечний» — усі паролі й дані форм на ньому передаються у відкритому вигляді, доступні для перехоплення." },
  { name: "порт (port)", desc: "Числовий «вхід» на сервері для конкретного типу трафіку — один сервер може одночасно обслуговувати вебсайт, пошту й базу даних на різних портах.", syntax: "example.com:443 — HTTPS\nexample.com:22 — SSH", attrs: "80 — HTTP, 443 — HTTPS, 22 — SSH, 5432 — PostgreSQL", pitfalls: "Порт, зайнятий іншою програмою (напр. localhost:3000 уже використовується) — типова помилка «address already in use» при запуску сервера локально." },
  { name: "файрвол (firewall)", desc: "Система, що фільтрує вхідний і вихідний мережевий трафік за заданими правилами — «охоронець», що пропускає лише дозволені з'єднання.", syntax: "Дозволити: порт 443 (HTTPS)\nЗаблокувати: порт 23 (застарілий Telnet)", attrs: "firewall — «вогняна стіна» (метафора з протипожежної стіни в будівлі)", pitfalls: "Занадто суворі правила файрвола можуть заблокувати легітимний трафік власного застосунку — типова причина «чому мій сервер не відповідає на запити ззовні»." },
  { name: "шифрування (encryption)", desc: "Перетворення даних у нечитабельний вигляд за допомогою ключа, щоб лише той, хто має правильний ключ, міг їх розшифрувати назад.", syntax: "\"привіт\" + ключ → \"x7#kL9$mQ\" (зашифровано)", attrs: "симетричне шифрування (один ключ) vs асиметричне (пара ключів: публічний+приватний)", pitfalls: "Шифрування захищає дані ПІД ЧАС передачі чи зберігання, але не захищає від крадіжки самого ключа чи від атаки на кінцевий пристрій, де дані вже розшифровані." },
  { name: "хеш (hash)", desc: "Одностороння функція, що перетворює дані будь-якого розміру на рядок фіксованої довжини — неможливо відновити оригінал з хешу, але той самий вхід завжди дає той самий хеш.", syntax: "hash(\"password123\") → \"ef92b778...\"", attrs: "MD5, SHA-256, bcrypt — популярні алгоритми хешування", pitfalls: "Хеш — НЕ шифрування: його не можна «розшифрувати» назад навіть з правильним ключем, бо ключа немає взагалі — це принципово інший, односторонній механізм." },
  { name: "двофакторна автентифікація (2FA)", desc: "Підтвердження особи двома різними способами замість одного — зазвичай пароль (щось, що ти знаєш) + код з телефону (щось, що в тебе є).", syntax: "Пароль + код із застосунку (Google Authenticator, SMS)", attrs: "2FA, MFA (multi-factor authentication) — узагальнена назва", pitfalls: "SMS-код як другий фактор вразливіший за застосунок-автентифікатор — SIM-картку можна «перехопити» через оператора (SIM swapping)." },
];
...
<p style="font-size:12px;color:#666;margin-top:6px;">Результат: процес 1234 (node server.js) примусово завершено</p>`,
  ...
  "fullstack-linux-network-archive": {
    badge: "Full Stack",
    title: "curl/wget, ssh/scp, tar/zip, env/export",
    whatIsIt: "Команди для мережевої взаємодії з командного рядка, безпечного підключення до віддалених серверів, архівації файлів і роботи зі змінними середовища сесії.",
    useCases: ["тестування API прямо з терміналу без окремого GUI-інструменту", "підключення до віддаленого сервера для деплою чи налагодження", "упаковка проєкту в архів для передачі чи бекапу", "тимчасова конфігурація через змінні середовища поточної сесії"],
    syntax: `curl -X POST https://api.example.com/users -d '{"name":"Оля"}'\nssh user@server.com\nscp file.txt user@server.com:/path/\ntar -czf archive.tar.gz project/`,
    attributes: [
      { name: "curl / wget", desc: "виконують HTTP-запити з командного рядка чи завантажують файли" },
      { name: "ssh user@host", desc: "підключається до віддаленого сервера через захищений протокол" },
      { name: "scp file user@host:path", desc: "копіює файли на/з віддаленого сервера через SSH-з'єднання" },
      { name: "tar -czf archive.tar.gz dir/", desc: "створює стиснутий архів директорії" },
      { name: "tar -xzf archive.tar.gz", desc: "розпаковує архів" },
      { name: "export VAR=value", desc: "задає змінну середовища для поточної сесії терміналу" },
    ],
    example: `<pre style="background:#f4f4f4;padding:10px;border-radius:6px;font-size:13px;">curl -s https://api.github.com/users/octocat | head -5</pre>
<p style="font-size:12px;color:#666;margin-top:6px;">Результат виконання:</p>
<pre style="background:#111;color:#0f0;padding:10px;border-radius:6px;font-size:13px;">{
  "login": "octocat",
  "id": 1,
  ...
  ...</pre>`,
    pitfalls: [
      "Передача пароля напряму в команді (curl -u user:password) — лишається в історії команд терміналу у відкритому вигляді.",
      "export змінної лише в поточній сесії терміналу — після закриття вікна значення зникає, для постійності потрібен файл профілю (.bashrc/.zshrc).",
      "scp/tar без перевірки шляху призначення — можна ненавмисно перезаписати існуючі файли на сервері чи локально.",
    ],
    related: ["fullstack-linux-permissions-process", "fullstack-dotenv"],
  },
  "fullstack-client-server": {
    badge: "Full Stack",
    title: "Клієнт-сервер, Frontend ↔ Backend ↔ Database",
    whatIsIt: "Базова архітектурна модель веб-застосунків: клієнт (браузер) надсилає запити серверу, сервер обробляє логіку і звертається до бази даних, потім повертає результат назад клієнту. Три шари з чіткими обов'язками кожного.",
    useCases: ["розуміння, де саме має жити конкретна логіка — валідація вводу (клієнт+сервер), бізнес-правила (сервер), зберігання (БД)", "проєктування, які дані клієнт може отримати напряму, а які лише через API сервера", "розмежування відповідальності між фронтенд- і бекенд-розробниками в команді"],
    syntax: `Браузер (Frontend)\n   ↕ HTTP-запити\nСервер (Backend)\n   ↕ SQL/запити\nБаза даних (Database)`,
    attributes: [
      { name: "Клієнт (client)", desc: "браузер чи мобільний застосунок — те, з чим взаємодіє користувач" },
      { name: "Сервер (server)", desc: "обробляє запити, виконує бізнес-логіку, звертається до бази даних" },
      { name: "База даних (database)", desc: "постійне зберігання даних, до якого має доступ лише сервер, не клієнт напряму" },
      { name: "API", desc: "чіткий контракт (набір ендпоінтів), через який клієнт і сервер спілкуються" },
    ],
    example: `<pre style="background:#f4f4f4;padding:10px;border-radius:6px;font-size:13px;">// Frontend: запит до сервера
const res = await fetch('/api/users/42');
const user = await res.json();

// Backend: обробка запиту, звернення до БД
app.get('/api/users/:id', async (req, res) => {
  const user = await db.users.findById(req.params.id);
  res.json(user);
});</pre>
<p style="font-size:12px;color:#666;margin-top:6px;">Результат: клієнт ніколи напряму не звертається до бази даних — лише через API сервера</p>`,
    pitfalls: [
      "Прямий доступ фронтенду до бази даних (обхід сервера) — серйозна діра в безпеці, немає контролю доступу й валідації.",
      "Бізнес-логіка й перевірка прав лише на клієнті — будь-хто може обійти фронтенд і викликати API напряму, тому сервер завжди має перевіряти самостійно.",
      "Змішування відповідальності шарів (напр. SQL-запити прямо в компонентах фронтенду) — ускладнює підтримку й тестування.",
    ],
    related: ["backend-rest", "fullstack-microservices"],
  },
  "fullstack-microservices": {
    badge: "Full Stack",
    title: "Мікросервісна архітектура",
    whatIsIt: "Розбиває застосунок на кілька незалежних маленьких сервісів (кожен відповідає за одну область: користувачі, платежі, сповіщення) замість одного великого монолітного застосунку. Сервіси спілкуються між собою через API чи черги повідомлень.",
    useCases: ["незалежне масштабування частини системи з найбільшим навантаженням (напр. лише сервіс платежів)", "команди можуть розробляти й деплоїти свої сервіси незалежно одна від одної", "різні сервіси можуть використовувати різні технології, найкращі для конкретної задачі"],
    syntax: `users-service    (порт 3001)\norders-service   (порт 3002)\npayments-service (порт 3003)\n→ спілкуються через HTTP/черги повідомлень`,
    attributes: [
      { name: "моноліт (monolith)", desc: "протилежність мікросервісам — весь застосунок як один деплой" },
      { name: "сервіс (service)", desc: "незалежний застосунок, що відповідає за одну обмежену область відповідальності" },
      { name: "API Gateway", desc: "єдина точка входу, що маршрутизує запити клієнта до потрібного внутрішнього сервісу" },
      { name: "service discovery", desc: "механізм, за допомогою якого сервіси знаходять адреси одне одного в динамічному середовищі" },
    ],
    example: `<pre style="background:#f4f4f4;padding:10px;border-radius:6px;font-size:13px;">// API Gateway маршрутизує запит
GET /api/orders/17
  → orders-service отримує замовлення
  → звертається до users-service за іменем клієнта
  → повертає об'єднану відповідь клієнту</pre>
<p style="font-size:12px;color:#666;margin-top:6px;">Результат: клієнт робить один запит, не знаючи про внутрішню структуру з кількох сервісів</p>`,
    pitfalls: [
      "Перехід на мікросервіси для маленького проєкту «про запас» — величезне ускладнення інфраструктури без реальної потреби; моноліт часто краще для старту.",
      "Занадто дрібне дроблення сервісів — мережеві виклики між ними стають вузьким місцем швидкодії й джерелом складних збоїв.",
...
const TERM_GUIDES = {
  ...
  "server-github": {
    ...
    intro: "GitHub — хмарний хостинг git-репозиторіїв з інструментами командної роботи: рев'ю коду через Pull Request, автоматизація через Actions, відстеження задач через Issues.",
    simple: "GitHub — це місце, де живе твій код і його історія змін, доступне з будь-якого комп'ютера.",
    steps: [
      { title: "Реєстрація і створення репозиторію", code: `# 1. Зареєструйся на github.com\n# 2. New repository → назва → Create\ngit remote add origin https://github.com/user/repo.git\ngit push -u origin main`, resultLabel: "Результат:", result: `Репозиторій створено, локальний код відправлено на GitHub.` },
      { title: "Безкоштовний тариф і ліміти", code: `GitHub Free:\n- необмежена кількість публічних і приватних репозиторіїв\n- 2000 хв/міс GitHub Actions (приватні репо)\n- 500 MB Packages, 1 GB Git LFS`, resultLabel: "Що це означає:", result: `Для навчання й більшості пет-проєктів безкоштовного тарифу вистачає з запасом.` },
      { title: "Де взяти токен доступу (Personal Access Token)", code: `Settings → Developer settings →\nPersonal access tokens → Fine-grained tokens →\nGenerate new token`, resultLabel: "Результат:", result: `Токен показується ОДИН РАЗ одразу після створення — треба скопіювати негайно.`, note: "Обирай мінімальні права (scopes) — лише ті, що реально потрібні для задачі." },
      { title: "Як використати токен у коді", code: `# .env\nGITHUB_TOKEN=ghp_xxxxxxxxxxxxxxxxxxxx\n\n// у коді (Node.js)\nfetch('https://api.github.com/user', {\n  headers: { Authorization: \`Bearer \${process.env.GITHUB_TOKEN}\` }\n});`, resultLabel: "Результат:", result: `Запит до GitHub API виконується від імені твого акаунта.`, note: "Ніколи не комітьте .env із токеном у git — додайте .env у .gitignore." },
    ],
    usage: ["зберігання коду і його історії", "командна робота через Pull Request і рев'ю коду", "автоматизація тестів/деплою через GitHub Actions", "безкоштовний хостинг статичних сайтів через GitHub Pages"],
  },
  "server-cloudflare": {
    accent: "orange",
    titleEn: "Cloudflare",
    ...
    intro: "Cloudflare — CDN і захист сайту (DDoS, DNS), а також платформа для безкоштовного деплою фронтенду (Pages), серверless-функцій на межі мережі (Workers) і об'єктного сховища без плати за вихідний трафік (R2).",
    simple: "Cloudflare — це і захист сайту, і місце, де можна безкоштовно розмістити фронтенд чи маленький бекенд-скрипт (Worker).",
    steps: [
      { title: "Реєстрація і підключення проєкту", code: `npm i -g wrangler\nwrangler login\nwrangler deploy`, resultLabel: "Результат:", result: `Worker чи Pages-проєкт задеплоєно на *.workers.dev чи *.pages.dev.` },
      { title: "Безкоштовний тариф і ліміти", code: `Workers Free:\n- 100 000 запитів/день\nPages Free:\n- необмежена кількість сайтів, 500 збірок/міс\nR2 Free:\n- 10 GB сховища, 0 плати за вихідний трафік`, resultLabel: "Що це означає:", result: `Для більшості невеликих і середніх проєктів безкоштовного тарифу достатньо.` },
      { title: "Де взяти API-токен", code: `My Profile → API Tokens →\nCreate Token → обрати шаблон прав`, resultLabel: "Результат:", result: `Токен видається одразу — копіюєш і зберігаєш у безпечному місці.`, note: "Обирай токен з правами лише для конкретного сервісу (Workers/Pages/DNS), а не Global API Key." },
      { title: "Як використати токен у коді", code: `# .env\nCLOUDFLARE_API_TOKEN=xxxxxxxxxxxx\n\n// wrangler.toml для Worker бере токен з середовища автоматично`, resultLabel: "Результат:", result: `CLI (wrangler) і API-запити до Cloudflare автентифікуються цим токеном.`, note: "Global API Key (старий формат) дає повний доступ до акаунта — уникай його, використовуй скоповані токени." },
    ],
    usage: ["прискорення й захист сайту (CDN + DDoS-захист)", "безкоштовний хостинг фронтенду (Pages)", "серверless-функції на межі мережі, близько до користувача (Workers)", "дешеве об'єктне сховище файлів без плати за трафік (R2)"],
  },
  "server-supabase": {
    accent: "lime",
    titleEn: "Supabase",
    ...
    intro: "Supabase — відкритий Backend-as-a-Service на базі Postgres: готова база даних, автентифікація користувачів, файлове сховище й автогенероване REST/GraphQL API, без написання власного сервера.",
    simple: "Supabase — готовий бекенд: база даних Postgres, логін користувачів і файлове сховище, без написання власного сервера.",
    steps: [
      { title: "Реєстрація і створення проєкту", code: `New Project → назва, пароль до БД, регіон → Create new project`, resultLabel: "Результат:", result: `За кілька хвилин створюється готова Postgres-база з автогенерованим API.` },
      { title: "Безкоштовний тариф і ліміти", code: `Supabase Free:\n- 500 MB бази даних\n- 1 GB файлового сховища\n- 50 000 активних користувачів автентифікації/міс`, resultLabel: "Що це означає:", result: `Проєкт на безкоштовному тарифі призупиняється після тижня без активності — легко відновити одним кліком.` },
      { title: "Де взяти ключі", code: `Project Settings → API →\nProject URL, anon public key, service_role key`, resultLabel: "Результат:", result: `Три значення: адреса проєкту й два ключі з різним рівнем доступу.`, note: "anon key безпечний для фронтенду (працює разом з правилами доступу до рядків RLS)." },
      { title: "Як підключити ключі у коді", code: `# .env\nSUPABASE_URL=https://xxxx.supabase.co\nSUPABASE_ANON_KEY=eyJhbGc...\n\nimport { createClient } from '@supabase/supabase-js'\nconst supabase = createClient(\n  process.env.SUPABASE_URL,\n  process.env.SUPABASE_ANON_KEY\n);`, resultLabel: "Результат:", result: `Клієнт підключено — можна одразу читати/писати дані в таблиці Postgres.`, note: "service_role key дає ПОВНИЙ доступ в обхід правил безпеки — лише на сервері, ніколи у фронтенді." },
    ],
    usage: ["готова Postgres база без адміністрування сервера", "автентифікація користувачів (email/OAuth) без власної реалізації", "файлове сховище для аватарок і документів", "миттєве REST/GraphQL API поверх власних таблиць"],
  },
  "server-firebase": {
    accent: "amber",
    titleEn: "Firebase",
    ...
    linkLabel: "firebase.google.com",
    intro: "Firebase — платформа Google для швидкої розробки застосунків: NoSQL-база даних у реальному часі (Firestore), автентифікація, хостинг і файлове сховище — усе в одній консолі.",
    simple: "Firebase — це набір готових бекенд-сервісів від Google: база даних, логін користувачів і хостинг, під одним дахом.",
    steps: [
      { title: "Реєстрація і створення проєкту", code: `Firebase Console → Add project →\nназва проєкту → Continue → Create project`, resultLabel: "Результат:", result: `Створено проєкт з доступом до Firestore, Auth, Storage й Hosting.` },
      { title: "Безкоштовний тариф і ліміти", code: `Firebase Spark (безкоштовний):\n- 1 GB сховища Firestore, 50 000 читань/день\n- необмежена автентифікація\n- 10 GB хостингу й трафіку/міс`, resultLabel: "Що це означає:", result: `Для навчання й невеликих проєктів безкоштовного тарифу вистачає надовго.` },
      { title: "Де взяти конфігурацію проєкту", code: `Project settings → General →\nSDK setup and configuration → Config`, resultLabel: "Результат:", result: `Копіюється об'єкт firebaseConfig з apiKey, projectId та іншими полями.`, note: "apiKey Firebase НЕ секретний — доступ контролюється Security Rules, а не приховуванням ключа." },
      { title: "Як підключити конфігурацію в коді", code: `// firebase.js\nimport { initializeApp } from 'firebase/app'\n\nconst firebaseConfig = {\n  apiKey: import.meta.env.VITE_FIREBASE_API_KEY,\n  projectId: "my-app",\n};\nconst app = initializeApp(firebaseConfig);`, resultLabel: "Результат:", result: `Firebase SDK підключено — можна працювати з Firestore, Auth тощо.`, note: "Реальний захист даних — це Security Rules у консолі Firebase, а не .env." },
    ],
    usage: ["база даних у реальному часі для чатів і живих оновлень", "автентифікація користувачів (email, Google, телефон)", "хостинг фронтенд-застосунку", "push-сповіщення для мобільних і веб-застосунків"],
  },
  "server-elevenlabs": {
    accent: "cyan",
    titleEn: "ElevenLabs",
    ...
    intro: "ElevenLabs — сервіс генерації реалістичної озвучки (text-to-speech) через API: перетворює текст на голосове аудіо десятками різних голосів і мов.",
    simple: "ElevenLabs — це API, що перетворює написаний текст на аудіо з реалістичним людським голосом.",
    steps: [
      { title: "Реєстрація і вибір голосу", code: `elevenlabs.io/sign-up →\nVoice Library → обрати голос → скопіювати voice_id`, resultLabel: "Результат:", result: `Отримано ідентифікатор голосу для використання в запитах API.` },
      { title: "Безкоштовний тариф і ліміти", code: `Free tier:\n- 10 000 символів/міс\n- обмежений вибір голосів\n- використання дозволене лише не комерційно`, resultLabel: "Що це означає:", result: `Достатньо для тестування й невеликих особистих проєктів, не для продакшн-навантаження.` },
      { title: "Де взяти API-ключ", code: `Profile → API Keys → Create API Key`, resultLabel: "Результат:", result: `Ключ формату дов_...  показується один раз — копіюєш і зберігаєш.` },
      { title: "Як використати ключ у коді", code: `# .env\nELEVENLABS_API_KEY=xxxxxxxxxxxx\n\nconst res = await fetch(\n  \`https://api.elevenlabs.io/v1/text-to-speech/\${voiceId}\`,\n  {\n    method: 'POST',\n    headers: {\n      'xi-api-key': process.env.ELEVENLABS_API_KEY,\n      'Content-Type': 'application/json',\n    },\n    body: JSON.stringify({ text: 'Привіт, світ!' }),\n  }\n);`, resultLabel: "Результат:", result: `Відповідь містить аудіофайл (MP3) з озвученим текстом.`, note: "Запити з ключем виконуй лише з бекенду — інакше будь-хто побачить ключ і використає твій ліміт символів." },
    ],
    usage: ["озвучка для відео чи подкастів без диктора", "голосові асистенти й чат-боти", "аудіоверсії статей і книг", "локалізація контенту різними мовами й голосами"],
  },
  "server-vercel": {
    accent: "sky",
    titleEn: "Vercel",
    ...
    intro: "Vercel — платформа для деплою фронтенд-застосунків (особливо Next.js, її творці) прямо з git-репозиторію, з автоматичним preview-деплоєм для кожного Pull Request.",
    simple: "Vercel — це деплой сайту одним push у git: сама збирає, розміщує й видає посилання.",
    steps: [
      { title: "Реєстрація і підключення репозиторію", code: `vercel.com → Add New Project →\nImport Git Repository → Deploy`, resultLabel: "Результат:", result: `Застосунок задеплоєно на *.vercel.app, кожен наступний push оновлює його автоматично.` },
      { title: "Безкоштовний тариф і ліміти", code: `Hobby (безкоштовний):\n- 100 GB трафіку/міс\n- необмежена кількість деплоїв\n- лише для особистих некомерційних проєктів`, resultLabel: "Що це означає:", result: `Ідеально для портфоліо й пет-проєктів; для комерційного використання потрібен платний план Pro.` },
      { title: "Де взяти токен доступу", code: `Account Settings → Tokens →\nCreate Token`, resultLabel: "Результат:", result: `Токен для CLI/API-деплоїв поза звичайним git-інтеграційним флоу.` },
      { title: "Як додати змінні середовища в проєкт", code: `# локально\nvercel env add DATABASE_URL\n\n# або: Project Settings → Environment Variables`, resultLabel: "Результат:", result: `Змінна стає доступною як process.env.DATABASE_URL під час збірки й виконання на Vercel.`, note: "Секрети додавай через Environment Variables у дашборді, а не хардкодь у коді." },
    ],
    usage: ["хостинг React/Next.js/Vue-застосунків", "автоматичний preview-деплой для рев'ю кожного PR", "serverless-функції (API routes) поруч із фронтендом", "аналітика продуктивності сайту з коробки"],
  },
  "server-netlify": {
    accent: "teal",
    titleEn: "Netlify",
    ...
    linkLabel: "netlify.com",
    intro: "Netlify — платформа для хостингу статичних сайтів і фронтенд-застосунків з автоматичним деплоєм із git, формами без бекенду й serverless-функціями.",
    simple: "Netlify — як Vercel: підключаєш git-репозиторій, і сайт автоматично публікується при кожному push.",
    steps: [
      { title: "Реєстрація і підключення репозиторію", code: `netlify.com → Add new site →\nImport an existing project → обрати репозиторій`, resultLabel: "Результат:", result: `Сайт задеплоєно на *.netlify.app, наступні push автоматично оновлюють його.` },
      { title: "Безкоштовний тариф і ліміти", code: `Free tier:\n- 100 GB трафіку/міс\n- 300 хвилин збірки/міс\n- 125 000 викликів serverless-функцій/міс`, resultLabel: "Що це означає:", result: `Вистачає для більшості особистих проєктів і невеликих продакшн-сайтів.` },
      { title: "Де взяти токен доступу", code: `User settings → Applications →\nPersonal access tokens → New access token`, resultLabel: "Результат:", result: `Токен для деплою через CLI чи API поза звичайним git-інтеграційним флоу.` },
      { title: "Як додати змінні середовища", code: `netlify env:set API_KEY xxxxxxxx\n\n# або: Site settings → Environment variables`, resultLabel: "Результат:", result: `Змінна стає доступною під час збірки й виконання функцій на Netlify.`, note: "Змінні з префіксом без VITE_/NEXT_PUBLIC_ лишаються приховані від фронтенд-коду під час збірки." },
    ],
    usage: ["хостинг статичних сайтів і SPA (React/Vue/Svelte)", "форми зворотного зв'язку без написання бекенду", "serverless-функції для простої бекенд-логіки", "автоматичний preview-деплой для кожного PR"],
  },
  "server-render": {
    accent: "violet",
    titleEn: "Render",
    ...
    linkLabel: "render.com",
    intro: "Render — хостинг повноцінних бекенд-застосунків, баз даних і фонових задач (cron jobs) з git — альтернатива власному серверу без ручного налаштування DevOps.",
    simple: "Render — це місце, де можна розмістити СЕРВЕРНИЙ застосунок (не лише фронтенд), так само просто, як на Vercel.",
    steps: [
      { title: "Реєстрація і створення сервісу", code: `render.com → New → Web Service →\nобрати репозиторій → Build & Start Command → Create`, resultLabel: "Результат:", result: `Бекенд-застосунок задеплоєно й доступний за адресою *.onrender.com.` },
      { title: "Безкоштовний тариф і ліміти", code: `Free tier:\n- веб-сервіси «засинають» після 15 хв без запитів\n- 750 годин/міс безкоштовного часу роботи\n- безкоштовна Postgres-база на 90 днів`, resultLabel: "Що це означає:", result: `Ідеально для тестування й демо; перший запит після сну повільний (холодний старт).` },
      { title: "Де взяти API-ключ", code: `Account Settings → API Keys → Create API Key`, resultLabel: "Результат:", result: `Ключ для керування сервісами через Render API чи CLI.` },
      { title: "Як додати змінні середовища", code: `# Dashboard → Service → Environment →\n# Add Environment Variable\n\n// у коді (Node.js)\nconst dbUrl = process.env.DATABASE_URL;`, resultLabel: "Результат:", result: `Застосунок на Render читає значення через звичайний process.env, як і локально.` },
    ],
    usage: ["хостинг Node.js/Python/Go бекенд-застосунків", "керовані бази даних Postgres/Redis", "фонові задачі за розкладом (cron jobs)", "Docker-контейнери без власного сервера"],
  },
  "server-railway": {
    accent: "rose",
    titleEn: "Railway",
    ...
    intro: "Railway — платформа для швидкого деплою бекенду й баз даних з git, з готовими шаблонами одним кліком (Postgres, Redis, Node.js, тощо).",
    simple: "Railway — деплой бекенду й бази даних за кілька кліків, з готовими шаблонами замість налаштування з нуля.",
    steps: [
      { title: "Реєстрація і створення проєкту", code: `railway.app → New Project →\nDeploy from GitHub repo → обрати репозиторій`, resultLabel: "Результат:", result: `Застосунок задеплоєно, Railway сам визначає команду збірки й запуску.` },
      { title: "Тарифи і ліміти", code: `Trial:\n- одноразовий $5 кредиту при реєстрації\nHobby (платний):\n- $5/міс базова плата + оплата за фактичне використання`, resultLabel: "Що це означає:", result: `На відміну від інших сервісів у списку, постійного безкоштовного тарифу немає — лише стартовий кредит.` },
      { title: "Де взяти токен проєкту", code: `Project Settings → Tokens →\nCreate Token`, resultLabel: "Результат:", result: `Токен для CI/CD чи CLI-деплою поза git-інтеграцією.` },
      { title: "Як додати змінні середовища", code: `railway variables set DATABASE_URL=postgres://...\n\n# або: Project → Variables → New Variable`, resultLabel: "Результат:", result: `Змінна одразу доступна застосунку через process.env.`, note: "Railway автоматично прокидає змінні між пов'язаними сервісами (напр. URL бази даних у застосунок)." },
    ],
    usage: ["швидкий старт бекенду з готовими шаблонами", "бази даних (Postgres/MySQL/Redis/MongoDB) одним кліком", "прототипування без налаштування інфраструктури з нуля", "монорепозиторії з кількома сервісами в одному проєкті"],
  },
  "server-mongodb-atlas": {
    accent: "lime",
    titleEn: "MongoDB Atlas",
    titleUa: "MongoDB Atlas",
    link: "https://www.mongodb.com/cloud/atlas",
    linkLabel: "mongodb.com/cloud/atlas",
    intro: "MongoDB Atlas — офіційний хмарний хостинг MongoDB (документної NoSQL-бази даних) з безкоштовним кластером назавжди, без потреби ставити базу на власний сервер.",
    simple: "Atlas — це MongoDB «в хмарі»: створюєш базу за кілька кліків, замість встановлення й адміністрування власного сервера.",
    steps: [
      { title: "Реєстрація і створення кластера", code: `cloud.mongodb.com → Build a Database →\nM0 Free → обрати регіон → Create`, resultLabel: "Результат:", result: `Безкоштовний кластер створено за кілька хвилин, готовий приймати з'єднання.` },
      { title: "Безкоштовний тариф і ліміти", code: `M0 (Free Forever):\n- 512 MB сховища\n- спільні (shared) обчислювальні ресурси\n- без плати за час — кластер не «засинає»`, resultLabel: "Що це означає:", result: `На відміну від багатьох безкоштовних баз, M0 не видаляється й не призупиняється — але продуктивність обмежена.` },
      { title: "Де взяти рядок підключення", code: `Database → Connect →\nDrivers → скопіювати Connection String`, resultLabel: "Результат:", result: `Рядок формату mongodb+srv://user:password@cluster.mongodb.net/`, note: "Пароль користувача бази задається окремо в Database Access — це не пароль акаунта Atlas." },
      { title: "Як підключити базу в коді", code: `# .env\nMONGODB_URI=mongodb+srv://user:pass@cluster.mongodb.net/mydb\n\nimport { MongoClient } from 'mongodb'\nconst client = new MongoClient(process.env.MONGODB_URI);\nawait client.connect();`, resultLabel: "Результат:", result: `Застосунок підключено до бази — можна читати й записувати документи.`, note: "У Network Access додай лише потрібні IP-адреси (чи 0.0.0.0/0 тимчасово для розробки, не для продакшену)." },
    ],
    usage: ["гнучке зберігання даних без жорсткої схеми таблиць", "прототипування, коли структура даних ще змінюється", "каталоги товарів з різними наборами характеристик", "застосунки, що вже використовують MongoDB як основну БД"],
  },
  "server-neon": {
    accent: "teal",
    titleEn: "Neon",
    ...
    linkLabel: "neon.tech",
    intro: "Neon — серверless Postgres із можливістю миттєво створювати гілки (branches) бази даних, як git-гілки коду — зручно для тестування міграцій і preview-середовищ.",
    simple: "Neon — це звичайний Postgres, але «безсерверний»: масштабується сам і вміє створювати копії бази за секунди.",
    steps: [
      { title: "Реєстрація і створення проєкту", code: `neon.tech → Create a project →\nназва, регіон, версія Postgres → Create`, resultLabel: "Результат:", result: `Готова Postgres-база створена за кілька секунд.` },
      { title: "Безкоштовний тариф і ліміти", code: `Free tier:\n- 0.5 GB сховища\n- автопауза бази при бездіяльності\n- 10 гілок (branches) бази даних`, resultLabel: "Що це означає:", result: `Достатньо для розробки й невеликих проєктів; перший запит після паузи трохи повільніший.` },
      { title: "Де взяти рядок підключення", code: `Project Dashboard → Connection Details →\nскопіювати Connection string`, resultLabel: "Результат:", result: `Рядок формату postgresql://user:password@ep-xxx.neon.tech/dbname`, note: "Кожна гілка бази даних (branch) має свій власний рядок підключення." },
      { title: "Як підключити базу в коді", code: `# .env\nDATABASE_URL=postgresql://user:pass@ep-xxx.neon.tech/mydb\n\nimport postgres from 'postgres'\nconst sql = postgres(process.env.DATABASE_URL);\nconst users = await sql\`SELECT * FROM users\`;`, resultLabel: "Результат:", result: `Запит виконується так само, як зі звичайним Postgres — Neon сумісний з усіма Postgres-бібліотеками.` },
    ],
    usage: ["звичайна Postgres-база без адміністрування сервера", "окрема гілка бази для кожного Pull Request (preview-середовища)", "тестування міграцій на копії продакшн-даних без ризику", "проєкти з нерегулярним навантаженням (оплата лише за використання)"],
  },
  "server-upstash": {
    accent: "orange",
    titleEn: "Upstash",
    ...
    linkLabel: "upstash.com",
    intro: "Upstash — серверless Redis і черги повідомлень з оплатою за фактичні запити (pay-per-request), без потреби тримати постійно запущений сервер Redis.",
    simple: "Upstash — це Redis (швидке сховище ключ-значення) для проєктів, де немає сенсу платити за сервер, що весь час увімкнений.",
    steps: [
      { title: "Реєстрація і створення бази", code: `upstash.com → Create Database →\nобрати регіон, тип (Regional/Global) → Create`, resultLabel: "Результат:", result: `Redis-база готова до використання за кілька секунд.` },
      { title: "Безкоштовний тариф і ліміти", code: `Free tier:\n- 500 000 команд/міс\n- 256 MB сховища\n- до 10 000 команд/добу без овербукінгу`, resultLabel: "Що це означає:", result: `Достатньо для кешування й сесій у невеликих і середніх проєктах.` },
      { title: "Де взяти ключі доступу", code: `Database → REST API →\nUPSTASH_REDIS_REST_URL, UPSTASH_REDIS_REST_TOKEN`, resultLabel: "Результат:", result: `Пара URL + токен для доступу через звичайні HTTP-запити (REST), без TCP-з'єднання.` },
      { title: "Як підключити в коді", code: `# .env\nUPSTASH_REDIS_REST_URL=https://xxx.upstash.io\nUPSTASH_REDIS_REST_TOKEN=xxxxxxxx\n\nimport { Redis } from '@upstash/redis'\nconst redis = new Redis({\n  url: process.env.UPSTASH_REDIS_REST_URL,\n  token: process.env.UPSTASH_REDIS_REST_TOKEN,\n});\nawait redis.set('user:1', 'Оля');`, resultLabel: "Результат:", result: `Значення збережено в Redis, доступне з будь-якого середовища через REST API.` },
    ],
    usage: ["кешування частих запитів до бази даних", "зберігання сесій користувачів", "rate limiting (обмеження кількості запитів)", "черги завдань для serverless-функцій (Vercel/Cloudflare Workers)"],
  },
  "server-resend": {
    accent: "sky",
    titleEn: "Resend",
    ...
    intro: "Resend — сучасне API для відправки транзакційних листів (підтвердження реєстрації, скидання пароля, чеки) прямо з коду застосунку, зроблене для розробників.",
    simple: "Resend — це спосіб надіслати email з коду застосунку, без налаштування власного поштового сервера.",
    steps: [
      { title: "Реєстрація і підтвердження домену", code: `resend.com/signup →\nDomains → Add Domain → додати DNS-записи`, resultLabel: "Результат:", result: `Домен підтверджено — листи відправляються від твоєї адреси (напр. no-reply@myapp.com).`, note: "Без підтвердженого домену можна тестувати лише з тестовою адресою onboarding@resend.dev." },
      { title: "Безкоштовний тариф і ліміти", code: `Free tier:\n- 3000 листів/міс\n- 100 листів/день\n- 1 підтверджений домен`, resultLabel: "Що це означає:", result: `Вистачає для більшості пет-проєктів і невеликих продуктів на старті.` },
      { title: "Де взяти API-ключ", code: `Dashboard → API Keys → Create API Key`, resultLabel: "Результат:", result: `Ключ формату re_xxxxxxxx показується один раз одразу після створення.` },
      { title: "Як надіслати лист із коду", code: `# .env\nRESEND_API_KEY=re_xxxxxxxxxxxx\n\nimport { Resend } from 'resend'\nconst resend = new Resend(process.env.RESEND_API_KEY);\n\nawait resend.emails.send({\n  from: 'app@myapp.com',\n  to: user.email,\n  subject: 'Підтвердження реєстрації',\n  html: '<p>Вітаємо!</p>',\n});`, resultLabel: "Результат:", result: `Лист надіслано отримувачу; статус доставки видно в Dashboard → Logs.` },
    ],
    usage: ["листи підтвердження реєстрації й скидання пароля", "сповіщення про замовлення, чеки, квитанції", "email-розсилки за подіями в застосунку (welcome-лист)", "заміна власного SMTP-сервера, який важко налаштувати надійно"],
  },
  "server-stripe": {
    accent: "violet",
    titleEn: "Stripe",
    ...
    intro: "Stripe — платформа для прийому онлайн-платежів: картки, підписки, рахунки й повернення коштів через єдине API, з готовими UI-компонентами для форми оплати.",
    simple: "Stripe — це спосіб прийняти оплату карткою на сайті, не працюючи напряму з банками й не зберігаючи дані карток самостійно.",
    steps: [
      { title: "Реєстрація і тестовий режим", code: `stripe.com/register →\nDashboard відкривається одразу в Test mode`, resultLabel: "Результат:", result: `Можна тестувати платежі тестовими картками (напр. 4242 4242 4242 4242) без реальних грошей.` },
      { title: "Вартість використання", code: `Плата за підключення: 0\nКомісія: ~2.9% + $0.30 за успішну транзакцію\n(точний відсоток залежить від країни й способу оплати)`, resultLabel: "Що це означає:", result: `Платиш лише відсоток з реальних продажів — немає щомісячної плати за сам факт підключення.` },
      { title: "Де взяти ключі API", code: `Developers → API keys →\nPublishable key (pk_...) та Secret key (sk_...)`, resultLabel: "Результат:", result: `Два ключі: публічний для фронтенду, секретний — лише для бекенду.` },
      { title: "Як створити платіж у коді", code: `# .env (лише на сервері!)\nSTRIPE_SECRET_KEY=sk_test_xxxxxxxx\n\nconst stripe = require('stripe')(process.env.STRIPE_SECRET_KEY);\nconst session = await stripe.checkout.sessions.create({\n  line_items: [{ price: 'price_xxx', quantity: 1 }],\n  mode: 'payment',\n  success_url: 'https://myapp.com/success',\n});`, resultLabel: "Результат:", result: `Створено сесію оплати — користувача перенаправляють на готову сторінку Stripe Checkout.`, note: "Секретний ключ (sk_...) НІКОЛИ не має потрапляти у фронтенд-код чи git — лише змінні середовища сервера." },
    ],
    usage: ["прийом оплати карткою в інтернет-магазині", "платні підписки (SaaS-продукти)", "разові платежі й донати", "виплати іншим сторонам (маркетплейси, фрилансери)"],
  },
  "server-openai": {
    accent: "teal",
    titleEn: "OpenAI",
    ...
    intro: "OpenAI API дає доступ до мовних моделей (GPT) для генерації й розуміння тексту, а також до генерації зображень (DALL·E) — для інтеграції ШІ у власний застосунок.",
    simple: "OpenAI API — це спосіб додати в свій застосунок «мозок», що розуміє й генерує текст, як ChatGPT.",
    steps: [
      { title: "Реєстрація і поповнення балансу", code: `platform.openai.com/signup →\nBilling → Add payment method → поповнити баланс`, resultLabel: "Результат:", result: `Акаунт готовий робити запити до API — оплата списується за фактичне використання.` },
      { title: "Вартість і ліміти", code: `Оплата: за токени (частини слів) на вході й виході\nНові акаунти: часто отримують пробний кредит\nRate limits: залежать від рівня акаунта (tier)`, resultLabel: "Що це означає:", result: `Безкоштовного постійного тарифу немає — лише одноразовий пробний кредит для нових акаунтів.` },
      { title: "Де взяти API-ключ", code: `API keys → Create new secret key`, resultLabel: "Результат:", result: `Ключ формату sk-... показується один раз одразу після створення.` },
      { title: "Як використати ключ у коді", code: `# .env (лише на сервері!)\nOPENAI_API_KEY=sk-xxxxxxxxxxxx\n\nimport OpenAI from 'openai'\nconst openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });\n\nconst res = await openai.chat.completions.create({\n  model: 'gpt-4o-mini',\n  messages: [{ role: 'user', content: 'Привіт!' }],\n});`, resultLabel: "Результат:", result: `Відповідь моделі повертається в res.choices[0].message.content.`, note: "Ключ у фронтенд-коді видно будь-кому в DevTools — запити до OpenAI роби лише через власний бекенд." },
    ],
    usage: ["чат-боти й асистенти всередині застосунку", "автоматичне резюмування чи переклад тексту", "генерація зображень за текстовим описом", "аналіз і класифікація тексту (тональність, категорії)"],
  },
  "server-cloudinary": {
    accent: "cyan",
    titleEn: "Cloudinary",
    ...
    linkLabel: "cloudinary.com",
    intro: "Cloudinary — хостинг, оптимізація й трансформація зображень і відео «на льоту» через параметри в URL: зміна розміру, обрізка, конвертація формату без обробки на власному сервері.",
    simple: "Cloudinary — це місце для зображень і відео: завантажуєш файл один раз, а різні розміри й формати отримуєш просто змінюючи адресу.",
    steps: [
      { title: "Реєстрація і завантаження файлу", code: `cloudinary.com/users/register →\nMedia Library → Upload`, resultLabel: "Результат:", result: `Файл завантажено, доступний за унікальним URL із Cloudinary CDN.` },
      { title: "Безкоштовний тариф і ліміти", code: `Free tier:\n- 25 кредитів/міс (≈25 GB трафіку чи сховища)\n- 1 кредит = 1000 трансформацій зображень`, resultLabel: "Що це означає:", result: `Кредити витрачаються і на зберігання, і на трансформації — варто кешувати готові варіанти, а не генерувати заново щоразу.` },
      { title: "Де взяти ключі API", code: `Dashboard → Product Environment Credentials →\nCloud name, API Key, API Secret`, resultLabel: "Результат:", result: `Три значення: ім'я хмари (публічне) і пара ключів для підписаних (авторизованих) операцій.` },
      { title: "Як використати в коді", code: `// Зміна розміру просто через URL — без API-ключа:\n<img src="https://res.cloudinary.com/mycloud/image/upload/w_300,h_300,c_fill/photo.jpg" />\n\n# Завантаження файлу з бекенду (.env)\nCLOUDINARY_URL=cloudinary://key:secret@mycloud\n\ncloudinary.uploader.upload('photo.jpg');`, resultLabel: "Результат:", result: `Зображення показується обрізаним і зменшеним автоматично, без обробки на власному сервері.`, note: "API Secret потрібен лише для завантаження/видалення файлів із коду — сам перегляд зображень працює без нього." },
    ],
    usage: ["автоматичне стиснення й адаптація зображень під різні екрани", "обрізка й фільтри без встановлення бібліотек обробки зображень", "хостинг відео з адаптивним стрімінгом", "аватарки користувачів і галереї товарів"],
  },
  "server-sentry": {
    accent: "rose",
    titleEn: "Sentry",
    ...
    linkLabel: "sentry.io",
    intro: "Sentry — сервіс моніторингу помилок: автоматично перехоплює необроблені винятки в продакшені й показує повний стек-трейс, контекст користувача і що саме зламалось.",
    simple: "Sentry — це «чорна скринька» застосунку: коли щось падає в продакшені, ти дізнаєшся про це одразу, з усіма деталями.",
    steps: [
      { title: "Реєстрація і створення проєкту", code: `sentry.io/signup →\nCreate Project → обрати платформу (напр. Node.js/React)`, resultLabel: "Результат:", result: `Створено проєкт з унікальним DSN (адресою для відправки помилок).` },
      { title: "Безкоштовний тариф і ліміти", code: `Developer (безкоштовний):\n- 5000 помилок/міс\n- 1 користувач команди\n- 30-денне зберігання подій`, resultLabel: "Що це означає:", result: `Достатньо для одного розробника чи невеликого проєкту; для команди потрібен платний план.` },
      { title: "Де взяти DSN", code: `Project Settings → Client Keys (DSN) →\nскопіювати DSN`, resultLabel: "Результат:", result: `DSN у форматі https://xxxxx@o0.ingest.sentry.io/0`, note: "DSN НЕ секретний — його можна лишати прямо в клієнтському коді, він лише вказує, куди відправляти помилки." },
      { title: "Як підключити в коді", code: `// .env\nSENTRY_DSN=https://xxxxx@o0.ingest.sentry.io/0\n\nimport * as Sentry from '@sentry/node'\nSentry.init({ dsn: process.env.SENTRY_DSN });\n\n// Необроблені помилки тепер автоматично летять у Sentry`, resultLabel: "Результат:", result: `При падінні застосунку в Sentry з'являється сповіщення з повним стек-трейсом і контекстом.` },
    ],
    usage: ["виявлення помилок у продакшені раніше, ніж про них напишуть користувачі", "відстеження продуктивності (повільні запити, вузькі місця)", "моніторинг як фронтенд-, так і бекенд-помилок в одному місці", "алерти в Slack/email при сплеску нових помилок"],
  },
  "server-clerk": {
    accent: "amber",
    titleEn: "Clerk",
    ...
    linkLabel: "clerk.com",
    intro: "Clerk — готова автентифікація й керування користувачами (логін, реєстрація, профілі, сесії) як вбудовані UI-компоненти, замість написання власної системи логіну з нуля.",
    simple: "Clerk — це готовий логін/реєстрація для сайту: вставляєш компонент, а не пишеш власну автентифікацію.",
    steps: [
      { title: "Реєстрація і створення застосунку", code: `clerk.com → Create application →\nобрати способи входу (email, Google, GitHub)`, resultLabel: "Результат:", result: `Створено застосунок з готовими сторінками логіну й реєстрації.` },
      { title: "Безкоштовний тариф і ліміти", code: `Free tier:\n- 10 000 місячних активних користувачів (MAU)\n- необмежена кількість способів входу`, resultLabel: "Що це означає:", result: `Вистачає для більшості проєктів до появи серйозного масштабу користувачів.` },
      { title: "Де взяти ключі", code: `Dashboard → API Keys →\nPublishable key (pk_...) та Secret key (sk_...)`, resultLabel: "Результат:", result: `Публічний ключ для фронтенду, секретний — для перевірки сесій на бекенді.` },
      { title: "Як підключити в коді", code: `# .env\nVITE_CLERK_PUBLISHABLE_KEY=pk_test_xxxxxxxx\nCLERK_SECRET_KEY=sk_test_xxxxxxxx\n\n<ClerkProvider publishableKey={pubKey}>\n  <SignedIn><UserButton /></SignedIn>\n  <SignedOut><SignInButton /></SignedOut>\n</ClerkProvider>`, resultLabel: "Результат:", result: `На сторінці одразу з'являється готова кнопка входу/профілю користувача.`, note: "Secret key перевіряє сесії на бекенді — використовуй лише в серверному коді, ніколи у фронтенді." },
    ],
    usage: ["логін і реєстрація без написання власної системи автентифікації", "вхід через Google/GitHub/інші сервіси (OAuth) з коробки", "керування профілями й сесіями користувачів", "захист сторінок і API-маршрутів за статусом автентифікації"],
  },
};
